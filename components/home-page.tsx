...
import {
  Flame, TrendingUp, Zap, Clock, Target, Check, Plus,
  Dumbbell, Camera, X, ShieldCheck, AlertCircle, MessageCircle, Settings,
} from 'lucide-react'
      ...
      <div className="flex flex-1 items-center justify-between gap-1">
        {days.map((date) => {
          ...
          const dotColor =
            ...
            activity === 'nutrition' ? 'bg-emerald-500' : 'bg-secondary'
          return (
            <div key={key} className="flex flex-col items-center gap-1">
              ...
              <div className={cn(
                'h-2.5 w-2.5 rounded-full transition-all', dotColor,
                isToday && 'ring-2 ring-primary ring-offset-1 ring-offset-card',
                activity === 'none' && 'opacity-30',
              )} />
            </div>
          )
        })}
        ...
        <div className="relative">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Daily Challenge · Every Day</p>
          ...
          <p className="mt-2 text-sm text-muted-foreground">Complete 100 push-ups every day. AI camera tracks your reps automatically.</p>
          <div className="mt-3">
            ...
            <div className="mt-1.5 h-2 overflow-hidden rounded-full bg-secondary">
              <div className={cn('h-full rounded-full transition-all duration-500', completed ? 'bg-green-500' : 'bg-orange-500')}
...
function PushupCameraModal({ onClose, onComplete }: { onClose: () => void; onComplete: (count: number) => void }) {
  ...
  const onResults = useCallback((results: any) => {
    ...
    if (!results.poseLandmarks) { setFeedback('No body detected — step back so your full body is visible'); return }
    ...
    if (avgElbow < 100) {
      ...
      if (phaseRef.current === 'down') {
        ...
        if (pushupCountRef.current >= PUSHUP_GOAL) { setFeedback('100 reps done! Amazing!'); setTimeout(() => onComplete(pushupCountRef.current), 1500) }
  ...
  useEffect(() => {
    const loadAndStart = async () => {
      try {
        ...
        setStatus('error')
        setErrorMsg(err?.name === 'NotAllowedError' ? 'Camera permission denied — allow access in browser settings' : `Camera error: ${err?.message ?? 'unknown'}`)
      }
    }
    loadAndStart()
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-black">
      ...
      <div className="bg-black/90 px-4 pb-10 pt-4">
        <div className="flex items-center justify-between">
          ...
          <div className={cn('rounded-full px-3 py-1.5 text-xs font-bold', phase === 'down' ? 'bg-orange-500/20 text-orange-400' : 'bg-white/10 text-white/60')}>
...
function DailyQuestCard() {
  ...
  return (
    <div className={cn('relative overflow-hidden rounded-2xl border bg-card p-4', completed ? 'border-green-500/40' : 'border-primary/30')}>
      <div className={cn('pointer-events-none absolute -right-10 -top-10 h-32 w-32 rounded-full blur-3xl', completed ? 'bg-green-500/15' : 'bg-primary/15')} />
      <div className="relative">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Daily Quest · Changes Tomorrow</p>
        ...
        <div className="mt-3">
          <div className="flex items-center justify-between text-xs">
            <span className="text-muted-foreground">Progress</span>
            <span className="font-medium text-foreground">{progress} / {quest.target} {quest.unit}</span>
          </div>
          <div className="mt-1.5 h-2 overflow-hidden rounded-full bg-secondary">
            <div className={cn('h-full rounded-full transition-all duration-500', completed ? 'bg-green-500' : 'bg-primary')}
...
function MacroStat({ label, value, percent, barClass }: { label: string; value: string; percent: number; barClass: string }) {
  return (
      ...
      <section className="grid grid-cols-2 gap-3">
        ...
        <button type="button" onClick={() => onTabChange?.('train')}
          ...
          <span className="text-xs font-bold text-foreground">
            {todayWorkout ? `Train — ${todayWorkout.name}` : 'Rest day'}
          </span>
        </button>
      </section>

      {/* Rank card — score shown ONCE, clearly */}
      <div className={cn('relative overflow-hidden rounded-2xl border border-border bg-card p-4', currentRank.glowClass)}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <RankBadge symbol={currentRank.symbol} size={40} />
            <div>
          ...
          <div className="text-right">
            <p className="text-2xl font-black tabular-nums text-foreground">{performanceScore}</p>
            <p className="text-xs text-muted-foreground">strength pts</p>
          </div>
        </div>
        ...
        {nextRank && (
          <div className="mt-3 flex items-center justify-between rounded-xl bg-secondary/50 px-3 py-2.5">
            <div className="flex items-center gap-2">
              <RankBadge symbol={nextRank.symbol} size={22} />
              <span className="text-sm font-semibold" style={{ color: nextRank.color }}>{nextRank.name}</span>
            </div>
            <span className="text-xs text-muted-foreground">+{pointsToNext} pts to unlock</span>
          </div>
        )}
      </div>
